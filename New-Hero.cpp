...
int main()
{
	...
	Knight s[size];
	Knight s1;
	cout<<"type knight name: ";												// nhap ten anh hung
	getline(cin,s1.name);
	do{
...
do{
	...
	do{
		...
		cout<<"\n~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"<<endl;
		}while (lowmp=="lowmp");							//low mp --> return action (tiger)
		crit_rate= rand() % 100;							//crit rate (knight)
		if (crit_rate<s1.crr)
			crit=1;
		else
			crit=0;
		cout<<"\n\n\ntiger:";
		s2.be_attacked(dame,mdame,crit);					//be attacked
		dame=0;												//return variable
		mdame=0;
		crit=0;
		do{	action= rand() % 3 + 1;							//Tiger action
			lowmp="000";
			switch (action){
				case 1:
					dame=s2.atk;
			...
			crit_rate= rand() % 100;							//crit rate (tiger)
			if (crit_rate<s2.crr)
				crit=1;
			else
				crit=0;
